...
describe('vite-plugin-saad', () => {
  ...
  it('generates multi-runtime wrapper with detection logic', async () => {
    ...
    expect(wrapperContents).toContain("const isDeno = typeof Deno !== 'undefined'");
    expect(wrapperContents).toContain("const isBun = typeof Bun !== 'undefined'");

    // Check for Deno branch
    expect(wrapperContents).toContain('if (isDeno)');
    expect(wrapperContents).toContain("await import('npm:@hono/deno@^1.0.0')");
    ...
    expect(wrapperContents).toContain('else if (isBun)');
    expect(wrapperContents).toContain("await import('hono/bun')");
    expect(wrapperContents).toContain('(Bun)');

    // Check for Node.js branch (else)
    expect(wrapperContents).toContain('else {');
    expect(wrapperContents).toContain("await import('@hono/node-server')");
    expect(wrapperContents).toContain("await import('@hono/node-server/serve-static')");
